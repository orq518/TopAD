...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
    ...
    protected static final int TIMEOUT_MESSAGE=7;
	...
	if (!missingLooper)
		...
		LogUtil.d(LOG_TAG, "Current thread has not called Looper.prepare(). Forcing synchronous mode.");
	}
     ...
     */
    public void onProgress(int bytesWritten, int totalSize) {
        LogUtil.d(LOG_TAG, String.format("Progress %d from %d (%2.0f%%)", bytesWritten, totalSize, (totalSize > 0) ? (bytesWritten * 1.0 / totalSize) * 100 : -1));
    }
    ...
    public void onRetry(int retryNo) {
        LogUtil.d(LOG_TAG, String.format("Request retry no. %d", retryNo));
    }

    public void onCancel() {
        LogUtil.d(LOG_TAG, "Request got cancelled");
    }

    public void onTimeout(){
        LogUtil.d(LOG_TAG, "request time out");
    }
    ...
    final public void sendTimeoutMessage(){
    	sendMessage(obtainMessage(TIMEOUT_MESSAGE,null));
    }
        ...
        switch (message.what) {
            case SUCCESS_MESSAGE:
                ...
                if (response != null && response.length >= 3) {
                    ...
                    LogUtil.d(LOG_TAG, "SUCCESS_MESSAGE didn't got enough params");
                }
                break;
            case FAILURE_MESSAGE:
                ...
                if (response != null && response.length >= 4) {
                    ...
                    LogUtil.d(LOG_TAG, "FAILURE_MESSAGE didn't got enough params");
                }
                break;
            case START_MESSAGE:
                onStart();
            ...
            case PROGRESS_MESSAGE:
                ...
                if (response != null && response.length >= 2) {
                    try {
                        ...
                        LogUtil.d(LOG_TAG, "custom onProgress contains an error");
                    }
                } else {
                    LogUtil.d(LOG_TAG, "PROGRESS_MESSAGE didn't got enough params");
                }
                break;
            case RETRY_MESSAGE:
                response = (Object[]) message.obj;
                if (response != null && response.length == 1)
                    ...
                    LogUtil.d(LOG_TAG, "RETRY_MESSAGE didn't get enough params");
                break;
            case CANCEL_MESSAGE:
                onCancel();
                break;
